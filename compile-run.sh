...
for i in `seq 1 16`; do
    ...
    echo "Executando o mandelbrot${i}.in com MPI apenas"
    exec_time=$( { time mpirun --machinefile hosts.txt --mca btl_tcp_if_include 10.20.221.0/24 bin/mandelbrot-mpi-only.out < inputs/mandelbrot${i}\ copy.in > /dev/null; } 2>&1 )
    echo "$exec_time" >> times.txt
    echo $exec_time
...
for i in `seq 1 16`; do
    ...
    echo "Executando o mandelbrot${i}.in com MPI e OpenMP"
    exec_time=$( { time mpirun -bind-to none --machinefile hosts.txt --mca btl_tcp_if_include 10.20.221.0/24 bin/mandelbrot-with-omp.out < inputs/mandelbrot${i}\ copy.in > /dev/null; } 2>&1 )
    echo $exec_time
    echo "$exec_time" >> times.txt
